...
Public Class PropertiesForm
    ...
    Private PathFileCopy As String = ""

    'Abre las configuraciones del proyecto.
    Private Sub OpenConfig(ByVal Path As String)
        ...
        For Each persona As XmlElement In Nodos.ChildNodes
            TB_Ico.Text = persona.Item("Icon").InnerText
    ...
    Private Sub SaveSetting(ByVal Path As String)
        ...
        'Guarda Icono. 
        Dim Ico As XmlElement = Documento_xml.CreateElement("Icon")
    ...
    Private Sub UpdateVLP()
        ...
        For Each persona As XmlElement In personas.ChildNodes
            Me.TB_Ico.Text = persona.Item("Icon").InnerText
            Me.TB_Author.Text = persona.Item("Author").InnerText
            Me.TB_Version.Text = persona.Item("Version").InnerText
            Me.TB_Company.Text = persona.Item("Company").InnerText
            Me.TB_DateCreate.Text = persona.Item("Date").InnerText

            Me.CB_LoveVersion.SelectedItem = "Love " & persona.Item("LoveVersion").InnerText
        Next
    End Sub
    ...
    Private Sub PropertiesForm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            UpdateVLP()
            UpdateProject1.NameProject = UpdateProject1.RemoveExtencion(My.Computer.FileSystem.GetName(MainForm1.TSSL_Path.Text))

            PB_Ico.Image = UpdateProject1.OpenImageToLiberate(TB_Ico.Text)
    ...
    Private Sub BT_Accet_Click(sender As System.Object, e As System.EventArgs) Handles BT_Accet.Click
        Dim LoveVersion As String = CB_LoveVersion.SelectedItem
        MainForm1.LoveVersionProject = LoveVersion.Replace("Love ", "")

        UpdateProject1.NameProject = UpdateProject1.RemoveExtencion(My.Computer.FileSystem.GetName(MainForm1.TSSL_Path.Text))
        UpdateProject1.LoveVersion = LoveVersion.Replace("Love ", "")
        UpdateProject1.AppVersion = TB_Version.Text
        UpdateProject1.NameAuthor = TB_Author.Text
        UpdateProject1.NameCompany = TB_Company.Text
        UpdateProject1.PathIcon = TB_Ico.Text

        'UpdateProject1.TimerCreate = My.Computer.Clock.LocalTime.ToString
        'UpdateProject1.SaveLoveSetting()

        UpdateProject1.ModifyLoveSetting()

        If PathFileCopy = "" Then
        Else
            UpdateProject1.CopyIconToFolderProject(PathFileCopy, True)
        End If
    ...
    Private Sub BT_Shared_Click(sender As System.Object, e As System.EventArgs) Handles BT_Shared.Click
        Dim openFile As New OpenFileDialog
        openFile.Title = ""
        openFile.FileName = Nothing
        openFile.Filter = "icon file|*.ico*"
        '------------------------------------------------------------
        If openFile.ShowDialog = Windows.Forms.DialogResult.OK Then
            Try
                TB_Ico.Text = UpdateProject1.CopyIconToFolderProject(openFile.FileName, False)

                PB_Ico.Image = Image.FromFile(openFile.FileName)

                PathFileCopy = openFile.FileName
